...
void buildCharacter(Player& player);//make
bool verifyRole(int& Cchoice);//roles select
void view_character_information(Player& player);
void menu();
void decidechoice(Player& player);
bool reviewchoice(int& primarychoice);//check
void beginmission(Player& player);
bool questchoice(int& qchoice);
void combat(Player& player, Quest& quest);//hit = attack 
//q_intensity -- quest difficulty
bool attackchoice(int& attack_choice);
void Handle_Attack_Choice(int attack_choice, int& playerstrike, Player& player);
...
int main()
{
    ...
    while (1)
    {
        ...
        view_character_information(Character);
        menu();
        decidechoice(Character);
    }
...
void buildCharacter(Player& player)
{
...
void decidechoice(Player& player)
{
    ...
    {
    case 1:
        beginmission(player);//go on mission
        break;
    case 2:
        cout << "\n\nSee you next time";
        exit(0);
...
void beginmission(Player& player)//go on advanture /quest
{

    Quest sage_quests[] = {
                       {"Sage's Wisdom ", 1 , 25 , "Book of wisdom","Sauron"},

                       {"Retreieve the ancient scrolls", 2 , 50 , "Elixir of clarity","Saruman"},
...
void combat(Player& player, Quest& quest)
{
    ...
    do
    {
        ...
        Handle_Attack_Choice(attack_choice, playerstrike, player);
